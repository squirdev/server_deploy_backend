<template>
    <div>
        <div class="dabg">
            <div class="gebgh">
                <div>
                    <h2><span class="hbnh" @click="$router.go(-1)"><a class="fan"></a></span> 大宗交易记录 </h2>
                </div>
            </div>
            <div class="jilkm" v-for="(item, index) in shengouList" :key="index">
                <div class="namkl">
                    <div class="nlt">
                        ...
                        <p>
                            <span v-if="item.stockGid.indexOf('sz')>=0">深</span>
                            <span class="sh" v-if="(item.stockGid.indexOf('sh')>=0)">沪</span>
                            <span class="bj" v-if="(item.stockGid.indexOf('bj')>=0)">北</span>
                            <a
                    ...
                    <div class="rlt"> 购买价格 <span>{{ item.orderTotalPrice }}</span></div>
                    <div v-if="item.type == 1 && item.status == 3" @click="getrenjiao(item.id)" class="renjiao">认缴</div>
                </div>
                <div class="gnj"></div>
                <div class="plkm">
                    <p><span>购买数量</span><a>{{ item.orderNum }}</a></p>
                    <p><span>购买单价</span><a>{{ item.buyOrderPrice }}</a></p>
                    <p><span>购买订单</span><span>{{ item.buyOrderId }}</span></p>
                    <p><span>购买时间</span><span>{{ item.buyOrderTime | gettime }}</span></p>
                </div>
            </div>
        </div>
    </div>
